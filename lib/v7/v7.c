...
DIR *opendirF(const char *dir_name);
int closedirF(DIR *dir);
struct dirent *readdirF(DIR *dir);
...
/*
 * This file contains POSIX opendirF/closedirF/readdirF API implementation
...
DIR *opendirF(const char *dir_name) {
  ...
  if (dir_name != NULL && (dir = (DIR *) malloc(sizeof(*dir))) != NULL &&
      SPIFFS_opendirF(&fs, (char *) dir_name, &dir->dh) == NULL) {
    free(dir);
    dir = NULL;
  }
...
struct dirent *readdirF(DIR *dir) {
  return SPIFFS_readdirF(&dir->dh, &dir->de);
}
...
V7_PRIVATE enum v7_err File_list(struct v7 *v7, v7_val_t *res) {
  ...
  *res = V7_UNDEFINED;
  ...
  if (v7_is_string(arg0)) {
    ...
    if ((dirp = (opendirF(path))) != NULL) {
      *res = v7_mk_array(v7);
      while ((dp = readdirF(dirp)) != NULL) {
